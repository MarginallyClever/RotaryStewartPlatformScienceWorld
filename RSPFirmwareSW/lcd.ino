...
void LCD_status_menu() {
  MENU_START
    ...
    //lcd.setCursor( 0, 0);  lcd.print('X');  LCD_print_float(robot.ee.pos.x);
    //lcd.setCursor(10, 0);  lcd.print('Y');  LCD_print_float(robot.ee.pos.y);
...
void LCD_print_action() {
  long seconds = (millis() - countDownStart)/1000;
...
void LCD_print_time(long seconds) {
  long minutes = seconds / 60;
  seconds %=60;
  if(minutes>10) lcd.print((int)(minutes/10));
  else lcd.print(' ');
  lcd.print((int)(minutes%10));
  lcd.print(':');
  lcd.print((int)(seconds/10));
  lcd.print((int)(seconds%10));
}
...
void LCD_go_home() {
  robot_line( 0,0,0, 0,0,0, DEFAULT_FEEDRATE );
  MENU_GOTO(LCD_main_menu);
}
