...
static int sofar;  // how much is in the serialBuffer
...
float parseNumber(char code,float val) {
  ...
  while( ptr>(char *)1 && (int)(*ptr) && ptr<(serialBuffer+sofar) ) {
    if(*ptr==code) {
      return atof(ptr+1);
    }
    ptr=strchr(ptr,' ')+1;
...
/**
 * Look for character /code/ in the serialBuffer.
...
/**
 * Read the input serialBuffer and find any recognized commands.  One G or M command per line.
 */
...
/**
 * Prepares the input serialBuffer to receive a new message and tells the serial connected device it is ready for more.
 */
void parser_ready() {
...
void parser_listen() {
  ...
  while(Serial.available() > 0) {  // if something is available
    ...
    if(c=='\n') {
      serialBuffer[sofar]=0;  // end the serialBuffer so string functions work right
